...
create table if not exists xv_bid_l2_1_"""+table_name+"""(
    ...
    flow_type varchar(20) DEFAULT NULL COMMENT 'pc_banner,pc_text,pc_video,mobile_web,mobile_app,mobile_native',
...
def data_import(date):
    hive_sql = """ /data/hadoop/hive-1.1.0-cdh5.5.0/bin/hive -e "
        ...
        case when xv_bid_l2_1.channel_id>2000 then 1 else 0 end as trade_type,
        xv_bid_l2_1.channel_id, xv_bid_l2_1.project_id, xv_bid_l2_1.campaign_id, campaign.bidding_client_type, creative_id, is_rd, region_code, domain, subdomain, 
        ...
        from xview.xv_bid_l2_1
        join xview.channel on xv_bid_l2_1.channel_id=channel.id
        join xview.campaign on xv_bid_l2_1.project_id=campaign.project_id and xv_bid_l2_1.campaign_id=campaign.campaign_id

        where day_id='"""+ date +"""';
    " > /data/xview/bid_statistics/day_xv_bid_l2_1"""
    return command(hive_sql)

def load_mysql(date):
    mysql_sql = """ mysql -h 192.168.4.235 -udmpapadmin -p'dc077e28' -D xview -e "
    DELETE from xv_bid_l2_1_""" + date + """ where DATE_FORMAT(rep_date,'%Y%m%d')='"""+ date +"""';
    LOAD DATA LOCAL INFILE '/data/xview/bid_statistics/day_xv_bid_l2_1' INTO TABLE xv_bid_l2_1_""" + date + """( 
    rep_date, trade_type, channel_id, project_id, campaign_id, flow_type, creative_id, is_rd, region_code, domain, subdomain, 
    bid, imp, clk, rtb_cost, bid_cost, profit, order_num, order_price, arrival_num, pv, pv_duration, two_jump_num, reg_num, stop_time, 
    jump_num, uv, shop_cart, shop_cart_price, invalid_clk, rtb_price, active_num);
    " """
    return command(mysql_sql)
