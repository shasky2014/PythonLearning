...
insert overwrite table report.reqandunify_adslot_report partition (day_id='"""+date+"""')
select channel_id, flow_type, domain, subdomain, adsite_id, project_id, campaign_id, creative_id,
    ...
    sum(shop_cart_price) shop_cart_price, sum(invalid_click) invalid_click, sum(activate_num) activate_num
from(
    -- pc广告位请求数
    select channel_id,if(traffic_type=1,'mobile_web',flow_type) flow_type, domain, subdomain,adsite_id,null as project_id ,null as campaign_id,null as creative_id,
    sum(req_count) req_count,
        ...
        0 reg_num, 0 stop_time, 0 jump_num, 0 uv, 0 shop_cart, 0 shop_cart_price, 0 invalid_click, 0 activate_num
    from dw.dw_adx_req 
    where day_id = '"""+date+"""'
    group by if(traffic_type=1,'mobile_web',flow_type), channel_id, domain, subdomain, adsite_id

union all
    -- pc广告位竞价展现效果数
    select channel_id, bidding_client_type flow_type,domain, subdomain,adsite_id, a.proj_id as project_id, a.campaign_id, a.idea_id as creative_id,
        ...
        sum(shop_cart_price)shop_cart_price, sum(invalid_click) invalid_click, 0 activate_num
    from (select * from report.report_unify_adsite where day_id = '"""+date+"""') a 
    left outer join xview.campaign b
    on a.campaign_id=b.campaign_id
    group by channel_id, bidding_client_type,domain, subdomain,adsite_id, a.proj_id, a.campaign_id, a.idea_id

union all
    -- app广告位请求数
    select channel_id, if(adreq_type='' or adreq_type is null,'mobile_app','mobile_native') flow_type, 
    app_id as domain,app_name as subdomain,adslot_id adsite_id, null as project_id,null as campaign_id,null as creative_id,
    sum(req_count) req_count,
    ...
    0 shop_cart, 0 shop_cart_price,0 invalid_click,0 activate_num
    from report.report_app_statistics
    where day_id='"""+date+"""'
    group by channel_id,  if(adreq_type='' or adreq_type is null,'mobile_app','mobile_native'),  app_id, app_name,adslot_id

union all
    -- app广告位竞价展现效果数
    select channel_id, if(adreq_type='' or adreq_type is null,'mobile_app','mobile_native') flow_type, 
    app_id as domain,app_name as subdomain,adslot_id adsite_id, project_id, campaign_id,creative_id,
    ...
    0 shop_cart, 0 shop_cart_price,0 invalid_click,sum(active_count) activate_num
    from report.report_app_statistics
    where day_id='"""+date+"""'
    group by channel_id,  if(adreq_type='' or adreq_type is null,'mobile_app','mobile_native'),  app_id, app_name,adslot_id, project_id, campaign_id,creative_id
) t 
group by channel_id, flow_type, domain, subdomain, adsite_id, project_id, campaign_id, creative_id;
    " """
    print hive_sql
