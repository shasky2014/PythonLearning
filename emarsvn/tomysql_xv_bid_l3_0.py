...
create table if not exists xv_bid_l3_0_"""+table_name+"""(
...
def data_import(date):
    hive_sql = """ /data/hadoop/hive-1.1.0-cdh5.5.0/bin/hive -e "
        select from_unixtime(unix_timestamp(day_id,'yyyyMMdd'),'yyyy-MM-dd') as rep_date, domain, adslot_id, 
        ...
        from xview.xv_bid_l3_0
        where day_id='"""+ date +"""';
    " > /data/xview/bid_statistics/day_xv_bid_l3_0"""
    return command(hive_sql)

def load_mysql(date):
    mysql_sql = """ mysql -h 192.168.4.235 -udmpapadmin -p'dc077e28' -D xview -e "
DELETE from xv_bid_l3_0_""" + date + """ where DATE_FORMAT(rep_date,'%Y%m%d')='"""+ date +"""';
LOAD DATA LOCAL INFILE '/data/xview/bid_statistics/day_xv_bid_l3_0' INTO TABLE xv_bid_l3_0_""" + date + """( 
rep_date, domain, adslot_id, 
bid, imp, clk, rtb_cost, bid_cost, profit, order_num, order_price, arrival_num, pv, pv_duration, two_jump_num, reg_num, stop_time, 
jump_num, uv, shop_cart, shop_cart_price, invalid_clk, rtb_price, active_num) ;
    " """
    return command(mysql_sql)
